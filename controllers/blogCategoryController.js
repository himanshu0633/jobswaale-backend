...
exports.createBlogCategory = async (req, res) => {
  try {
    ...
    const nameExists = await BlogCategory.findOne(caseInsensitiveExactFilter('name', cleanName));
    if (nameExists) {
    ...
    if (sortingNo !== undefined && sortingNo !== null && sortingNo !== '') {
      const sortExists = await BlogCategory.findOne({ sortingNo: Number(sortingNo) });
      if (sortExists) {
...
exports.updateBlogCategory = async (req, res) => {
  try {
    ...
    if (cleanName) {
      const duplicate = await BlogCategory.findOne(caseInsensitiveExactFilter('name', cleanName, { _id: { $ne: uid } }));
      if (duplicate) {
        return res.status(400).json({ message: 'Blog category with this Name already exists' });
      }
    }
    ...
    if (sortingNo !== undefined && sortingNo !== null && sortingNo !== '') {
      const sortExists = await BlogCategory.findOne({ sortingNo: Number(sortingNo), _id: { $ne: uid } });
      if (sortExists) {
        return res.status(400).json({ message: 'Sort number is already taken. Please enter another number.' });
      }
    }
